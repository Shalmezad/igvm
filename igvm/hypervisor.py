...
class Hypervisor(Host):
    ...
    def _find_domain(self, vm):
        ...
        # the console. On some combinations of libvirt versions listAllDomains
        # fails after VM was shut down but works on retry.
        try:
    ...
    def migrate_vm(self, vm, target_hypervisor, offline, offline_transport, tx):
        ...
        if offline:
            ...
            if offline_transport=='drbd':
                self.start_drbd(vm, target_hypervisor, tx=tx)
    ...
    def start_drbd(self, vm, peer, tx=None):
        # Ensure that current domain name is used, this might be non-fqdn
        # one on source Hypervisor.
        domain = self._get_domain(vm)

        self.host_drbd = DRBD(self, VG_NAME, domain.name(), vm.fqdn, True, tx)
        self.peer_drbd = DRBD(peer, VG_NAME, vm.fqdn, vm.fqdn, False, tx)

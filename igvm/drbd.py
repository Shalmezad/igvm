...
class DRBD(object):
    def __init__(self, hv, vg_name, lv_name, vm_name, master_role, tx=None):
        ...
        # Copy of transaction for skipping rollback of copying of disk
        if master_role:
            self.pre_migration_tx = tx.copy_checkpoint()
    ...
    def wait_for_sync(self):
        ...
        # No way to rollback after this point - this stop procedure is the same
        # as rollback would be.
        if hasattr(self, 'pre_migration_tx'):
            self.tx.checkpoint(self.pre_migration_tx)
